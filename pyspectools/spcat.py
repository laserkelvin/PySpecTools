...
def peak2cat(
    ...
    if sortby not in ["Intensity", "Frequency"]:
        ...
        sys.exit()
    sorted_df = peaks_df.sort_values(by=sortby, ascending=False, inplace=True)
    # Calculate the number of shots based on intensities, converted to real units
    sorted_df["Norm Int"] = (10 ** sorted_df["Intensity"]) / (
        10 ** sorted_df["Intensity"].max()
    )
    # Take the strongest line as 10 shots
    sorted_df["Shots"] = 10.0 / sorted_df["Norm Int"] ** 2.0
...
def plot_pickett(cat_dataframe, verbose=True):
    ...
    cnorm = colors.Normalize(
        vmin=cat_dataframe["Lower state energy"].min(),
    ...
    if verbose is True:
        ...
        lineplot = ax.vlines(
            ...
            ymin=-10.0,  # set the minimum as arb. value
            ymax=cat_dataframe["Intensity"],  # set the height as predicted value
            colors=colormap.to_rgba(
                cat_dataframe["Lower state energy"].astype(float)
            ),  # color mapped to energy
        )

        ax.set_xlabel("Frequency (MHz)")
        ax.set_ylabel("Intensity")
        ax.set_ylim([cat_dataframe["Intensity"].min() * 1.1, 0.0])
...
if __name__ == "__main__":
    if len(sys.argv) < 2:
        ...
        print(
            "Optional arguments:\t [low] and [high] frequency thresholds, [intensity] threshold"
        )
        sys.exit()
    ...
    outputname = input(
        "Specify a name for the batch file (default: generated_batch.ftb)"
    )
    if outputname == "":
        outputname = "generated_batch.ftb"
    sortby = input(
        "What do you want to sort by: Intensities or Frequency (default: Frequency)"
    )
    if sortby == "":
        sortby = "Frequency"
